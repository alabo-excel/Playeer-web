...
const PlayerModal = ({
  ...
  const fetchHighlights = async () => {
    const res = await api.get(`/highlights/user/${player}`);
    console.log(res.data?.data);
    setHighlights(res.data?.data || []);
  };

  const fetchUserData = async () => {
    const response = await api.get(`/users/${player}`);
    if (response.data?.success && response.data?.data) {
      setData(response.data.data);
      setAchievements(response.data.data.achievements || []);
      setCertifications(response.data.data.certificates || []);
    }
  };
  ...
  const fetchProfile = async () => {
    try {
      ...
      await api.get(`/users/view/${player}`);
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto">
        <button onClick={() => router.back()} className="flex gap-3 mt-20">
          <ArrowLeft />
          <span>Back</span>
        </button>
