...
const SideNav = ({ open = false, onClose }: SideNavProps) => {
  ...
  const baseClass = 'fixed left-0 top-0 h-screen border-r border-gray p-4 flex flex-col bg-[#FCFCFC] z-40 transition-transform duration-300';
  const desktopClass = 'hidden md:flex w-[20%] mt-20';
  const mobileClass = `md:hidden w-64 mt-0 ${open ? 'translate-x-0' : '-translate-x-full'}`;
    ...
    <>
      ...
      >
        <div className='flex mb-4'>
          <img className='w-32 my-auto block lg:hidden' src="/images/logo-colored.png" alt="" />
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}>
                <button
                  className={`flex my-3 p-3 rounded-md w-full transition-colors duration-200 ${isActive
      ...
      {/* Sidebar for desktop */}
      <aside className={`${baseClass} ${desktopClass}`}
        aria-hidden={false}
      >
        <div className='flex-1'>
          {navItems.map((item) => {
