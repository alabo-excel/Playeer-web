    ...
    <section className='relative mx-auto'>
      <div className='flex p-4'>
        <div className='relative lg:block hidden w-1/2'>
          <img className='rounded-3xl h-full' src="/images/auth-img.png" alt="" />
          <div className='flex justify-between absolute top-10 left-10 right-10'>
            <button onClick={() => router.back()} className='bg-primary cursor-pointer rounded-full p-3 flex text-[#FCFCFC]'>
              <ArrowLeft />
              <span>Back</span>
            </button>
        ...
        <div className='lg:w-1/2 w-full my-auto h-screen flex flex-col'>
          <div className='flex lg:absolute justify-between mb-4 top-0 lg:w-[20%] w-full right-5 left-5 top-6 ml-auto p-4'>
            <img className='lg:hidden block w-24 my-auto' src="/images/logo-colored.png" alt="" />
            <div className='flex justify-between'>
              <Link href={'/auth/signup'}>
                <button className={`bg-[#E5F4FF] cursor-pointer lg:px-8 px-4 py-2 rounded-full text-[#0095FF] transition-colors`}>Sign up</button>
              </Link>
              <Link href={'/auth/login'}>
                <button className="bg-[#0095FF] cursor-pointer lg:px-8 px-4 py-2 ml-6 rounded-full text-white transition-colors">Login</button>
              </Link>
            </div>
          </div>
